...
export default async function TeacherSubjectsPage() {
  ...
  // Get teacher's class term assignments with detailed information
  const teacherClassTerms = await prisma.teacherClassTerm.findMany({
    ...
    orderBy: [
      { classTerm: { term: { session: { startDate: "desc" } } } },
      { classTerm: { term: { startDate: "desc" } } },
      { classTerm: { class: { name: "asc" } } },
    ],
  })
  ...
  const enrichedSubjectAssignments = teacherSubjects.map((teacherSubject) => {
    ...
    const relevantClassTerms = teacherClassTerms.filter((tct) =>
      tct.classTerm.classSubjects.some((cs) => cs.subject.id === teacherSubject.subject.id)
    )

    // Group by term for better organization
    const termGroups = relevantClassTerms.reduce((acc, tct) => {
      ...
      const safeStudents = tct.classTerm.students.map((sct) => {
        ...
        return {
          ...
          fullName: `${user.firstName || ''} ${user.lastName || ''}`.trim() || 'Unknown Student',
    ...
    return {
      ...
      totalStudents: relevantClassTerms.reduce((sum, tct) => sum + tct.classTerm.students.length, 0),
      currentTermAssignment: relevantClassTerms.find(tct => tct.classTerm.term.isCurrent),
    }
  })
  ...
  const summaryStats = {
    ...
    totalStudents: teacherClassTerms.reduce((sum, tct) => sum + tct.classTerm.students.length, 0),
    currentTermClasses: teacherClassTerms.filter(tct => tct.classTerm.term.isCurrent).length,
    currentTermSubjects: enrichedSubjectAssignments.filter(esa => esa.currentTermAssignment).length,
  }
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-gray-900">My Teaching Assignments</h1>
              <p className="text-lg text-gray-600 mt-1">
                Subjects and classes assigned to you across all terms
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Staff ID: {teacher.staffId}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
              <div className="text-center">
                <p className="text-2xl font-bold text-blue-600">{summaryStats.totalSubjects}</p>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
              <div className="text-center">
                <p className="text-2xl font-bold text-green-600">{summaryStats.currentTermClasses}</p>
                <p className="text-sm text-gray-600">Current Term Classes</p>
              </div>
            </div>

            <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
              <div className="text-center">
                <p className="text-2xl font-bold text-purple-600">{summaryStats.totalStudents}</p>
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 border border-blue-200 rounded-full">
              ...
              <span className="text-sm text-blue-900">{teacher.school.name} ({teacher.school.code})</span>
            </div>
            {teacher.department && (
              <div className="flex items-center gap-2 px-4 py-2 bg-purple-50 border border-purple-200 rounded-full">
                <span className="text-sm font-medium text-purple-700">Department:</span>
                <span className="text-sm text-purple-900">{teacher.department}</span>
              </div>
            )}
            {teacher.qualification && (
              <div className="flex items-center gap-2 px-4 py-2 bg-green-50 border border-green-200 rounded-full">
                <span className="text-sm font-medium text-green-700">Qualification:</span>
                <span className="text-sm text-green-900">{teacher.qualification}</span>
              </div>
            )}
            {currentTerm && (
              <div className="flex items-center gap-2 px-4 py-2 bg-orange-50 border border-orange-200 rounded-full">
                <span className="text-sm font-medium text-orange-700">Current Term:</span>
                <span className="text-sm text-orange-900">{currentTerm.session.name} - {currentTerm.name}</span>
              </div>
            )}
          </div>
        </div>
